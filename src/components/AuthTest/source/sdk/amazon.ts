...
const load = ({ appId, scope }) => new Promise((resolve) => {
  ...
  (window as any).onAmazonLoginReady = () => {
    (window as any).amazon.Login.setClientId(appId);
...
const checkLogin = () => new Promise((resolve, reject) => {
  (window as any).amazon.Login.authorize({ scope: amazonScopes }, (response) => {
    if (response.error) {
...
const logout = () => new Promise((resolve) => {
  (window as any).amazon.Login.logout();
...
const getProfile = (authResponse) => new Promise((resolve, reject) => {
  (window as any).amazon.Login.retrieveProfile(authResponse.access_token, (response) => {
    if (response.error) {
      return reject(rslError({
        provider: 'amazon',
        type: 'get_profile',
