...
const checkLogin = () => new Promise((resolve, reject) => {
  ...
  return resolve();
/*   if (!(window as any).IN.User.isAuthorized()) {
...
/*   (window as any).IN.User.authorize(() => checkLogin()
    .then(getProfile)
    .then(resolve)
    .catch(reject)); */
  return resolve();
...
const logout = () => new Promise((resolve) => {
  // (window as any).IN.User.logout(resolve);
  return resolve();
});
...
/* const generateUser = (response) => ({
  ...
  token: {
    accessToken: (window as any).IN.ENV.auth.oauth_token,
    expiresAt: timestampFromNow((window as any).IN.ENV.auth.oauth_expires_in)
  }
}); */
