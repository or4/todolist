...
export default class SocialLogin extends Component<any, any> {
  static propTypes = {
  ...
  handleSocialLoginInvokeSuccess(res) {
    ...
    switch (provider) {
      case 'google':
        ...
        const authResponse = (window as any).gapi.auth2.getAuthInstance().currentUser.get().getAuthResponse(true);
      ...
      case 'linkedin':
        userProfile = {
          id: (window as any).IN.ENV.auth.member_id,
        ...
        expiresIn.setSeconds(expiresIn.getSeconds() + (window as any).IN.ENV.auth.oauth_expires_in);
  ...
  handleLogin(e, obj) {
    ...
    if (provider === 'facebook') {
      ...
      (window as any).FB.login((response) => {
        ...
        (window as any).FB.api('/me', { fields: 'email,name,id,first_name,last_name,picture' }, (profileResponse) => {
      ...
      (window as any).IN.User.authorize((data) => {
        (window as any).IN.API.Profile('me').fields([
          'id',
          'firstName',
          'lastName',
          'pictureUrl',
  ...
  render() {
    return (
      <div id={this.id} onClick={this.handleLogin as any}>{this.props.children}</div>
    );
  }
}
